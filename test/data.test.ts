...
import { AVALANCHE_PANGOLIN_FACTORY_ADDRESS, AVALANCHE_PANGOLIN_INIT_CODE_HASH, BSC_PANCAKESWAP_FACTORY_ADDRESS, BSC_PANCAKESWAP_INIT_CODE_HASH, BSC_SAFESWAP_FACTORY_ADDRESS, BSC_SAFESWAP_INIT_CODE_HASH, ETHEREUM_SUSHISWAP_FACTORY_ADDRESS, ETHEREUM_SUSHISWAP_INIT_CODE_HASH, ETHEREUM_UNISWAP_FACTORY_ADDRESS, ETHEREUM_UNISWAP_INIT_CODE_HASH } from "../src/constants"
...
describe('data', () => {
  test('Pair Uniswap', async () => {
    ...
    const pair = await Fetcher.fetchPairData(WETH[ChainId.ETHEREUM_MAINNET], token, provider, ETHEREUM_UNISWAP_FACTORY_ADDRESS[ChainId.ETHEREUM_MAINNET], ETHEREUM_UNISWAP_INIT_CODE_HASH)
  ...
  test('Pair Sushiswap', async () => {
    const provider = getDefaultProvider(getNetwork(ChainId.ETHEREUM_MAINNET))
    const token = await Fetcher.fetchTokenData(ChainId.ETHEREUM_MAINNET, "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48") //USDC
    const pair = await Fetcher.fetchPairData(WETH[ChainId.ETHEREUM_MAINNET], token, provider, ETHEREUM_SUSHISWAP_FACTORY_ADDRESS[ChainId.ETHEREUM_MAINNET], ETHEREUM_SUSHISWAP_INIT_CODE_HASH)
  ...
  test('Pair Pangolin', async () => {
    ...
    const pair = await Fetcher.fetchPairData(WETH[ChainId.AVALANCHE_MAINNET], token, provider, AVALANCHE_PANGOLIN_FACTORY_ADDRESS[ChainId.AVALANCHE_MAINNET], AVALANCHE_PANGOLIN_INIT_CODE_HASH)
  ...
  test('Pair PancakeSwap', async () => {
    ...
    const pair = await Fetcher.fetchPairData(WETH[ChainId.BSC_MAINNET], token, provider, BSC_PANCAKESWAP_FACTORY_ADDRESS[ChainId.BSC_MAINNET], BSC_PANCAKESWAP_INIT_CODE_HASH)
  ...
  test('Pair SafeSwap', async () => {
    ...
    const pair = await Fetcher.fetchPairData(token2, token, provider, BSC_SAFESWAP_FACTORY_ADDRESS[ChainId.BSC_MAINNET], BSC_SAFESWAP_INIT_CODE_HASH)
